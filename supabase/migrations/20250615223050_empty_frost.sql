/*
  ...
  1. New Tables
    ...
    - `cake_designs`
      ...
      - `name` (text)
      - `shape` (text with check constraint)
      - `buttercream` (jsonb)
      - `toppings` (jsonb)
      - `top_text` (text)
    ...
    - `cake_tiers`
      - `id` (bigint, primary key)
      - `design_id` (bigint, foreign key to cake_designs)
      - `tier_order` (integer)
      - `flavor` (text)
      - `color` (text)
      - `frosting` (text)
    ...
    - `orders`
      ...
      - `baker_id` (bigint, foreign key to users, optional)
      - `design_id` (bigint, foreign key to cake_designs)
      ...
      - `price` (numeric)
      - `modification_requests` (text, optional)
      - `otp_code` (text, optional)
    ...
    - `quotes`
      ...
      - `baker_id` (bigint, foreign key to users)
      - `price` (numeric)
      - `modification_requests` (text, optional)
      - `message` (text)
      - `is_active` (boolean)
    ...
    - `messages`
      ...
      - `sender_type` (text with check constraint)
      - `message` (text)
  ...
  2. Security
    ...
    - Bakers can see posted orders and manage assigned orders
    - Order participants can access messages
...
DROP TRIGGER IF EXISTS update_cake_tiers_updated_at ON cake_tiers;
