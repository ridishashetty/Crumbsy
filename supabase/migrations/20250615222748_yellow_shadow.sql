/*
  # Complete database schema setup for Crumbsy

  1. New Tables
    - `users` - User accounts (buyers, bakers, admins)
    - `cake_designs` - Custom cake designs
    - `cake_tiers` - Individual tiers of cake designs
    - `orders` - Cake orders from buyers
    - `quotes` - Baker quotes for orders
    - `messages` - Chat messages between buyers and bakers
    - `portfolio_items` - Baker portfolio images
...
CREATE POLICY "Users can read own profile" ON users
  FOR SELECT USING (((uid())::text = (id)::text) OR (user_type = 'admin'::text));

CREATE POLICY "Users can update own profile" ON users
  FOR UPDATE USING ((uid())::text = (id)::text);
...
CREATE POLICY "Bakers can update assigned orders" ON orders
  FOR UPDATE USING ((uid())::text = (baker_id)::text);

-- Quotes policies
CREATE POLICY "Bakers can create quotes" ON quotes
  FOR INSERT WITH CHECK ((uid())::text = (baker_id)::text);
...
CREATE POLICY "Buyers can read quotes for own orders" ON quotes
  FOR SELECT USING (EXISTS ( SELECT 1
   FROM orders
  WHERE ((orders.id = quotes.order_id) AND ((orders.buyer_id)::text = (uid())::text))));

-- Messages policies
CREATE POLICY "Order participants can create messages" ON messages
  FOR INSERT WITH CHECK (((sender_id)::text = (uid())::text) AND (EXISTS ( SELECT 1
   FROM orders
  WHERE ((orders.id = messages.order_id) AND (((orders.buyer_id)::text = (uid())::text) OR ((orders.baker_id)::text = (uid())::text))))));
...
CREATE POLICY "Bakers can manage own portfolio" ON portfolio_items
  FOR ALL USING ((uid())::text = (baker_id)::text);
