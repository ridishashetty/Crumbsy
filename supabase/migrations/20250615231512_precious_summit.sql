/*
  ...
  1. New Tables
    - `cake_flavors` - Available cake flavors with colors
    - `frosting_types` - Available frosting types with customization options
    - `topping_types` - Available toppings with icons
    - `users` - User accounts (buyers, bakers, admins)
    - `cake_designs` - Custom cake designs created by users
    - `cake_tiers` - Individual tiers for each cake design
    - `orders` - Cake orders placed by buyers
    - `quotes` - Price quotes from bakers for orders
    - `messages` - Chat messages between buyers and bakers
    - `portfolio_items` - Baker portfolio showcase items
  ...
  3. Features
    - Complete enum types for data consistency
...
-- Create triggers for updated_at (ALL using the same function name)
